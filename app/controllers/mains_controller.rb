class MainsController < ApplicationController
  ...
  # GET /mains/1.json
  def show
#	@mains = Main.find_by_slug(params[:id])
...
#	  end  
  ...
  private
	...
	def auction_image_params
      params.require(:auction_image).permit(:auction_id, :imageattachment, :image_file_name)
    end
	
    # Never trust parameters from the scary internet, only allow the white list through.
    def main_params
	  params.require(:main).permit(:title, :slug, :menupunkt, :imageattachment, :body, :description, :keywords)	
	end
end
